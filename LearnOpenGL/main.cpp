...
int main()
{
	...
	Shader lightingShader("shaders/vertex.vert", "shaders/materialmap.frag");
	Shader lampShader("shaders/vertex.vert", "shaders/color.frag");

	stbi_set_flip_vertically_on_load(true);
	unsigned int diffuseMap = loadTexture("textures/container2.png");
	unsigned int specularMap = loadTexture("textures/gay.png");
...
// 1. bind Vertex Array Object
	...
	lightingShader.setInt("texture", 0); // or with shader class
	lightingShader.setInt("texture2", 1); // or with shader class

	myCamara = Camera(glm::vec3(0.0f, 0.0f, 3.0f));
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		view = glm::lookAt(myCamara.Position, myCamara.Position + myCamara.Front, myCamara.Up);
		projection = glm::perspective(glm::radians(myCamara.Zoom), (float)SCR_HEIGHT / (float)SCR_HEIGHT, 0.1f, 100.0f);
		...
		lightingShader.setVec3("viewPos", myCamara.Position);
		...
		lightingShader.setVec3("light.position", myCamara.Position);
		lightingShader.setVec3("light.direction", glm::vec3(0.0f,0.0f,-1.0f));
		lightingShader.setFloat("light.cutOff", glm::cos(glm::radians(12.5f)));
		lightingShader.setFloat("light.outerCutOff", glm::cos(glm::radians(17.5f)));

		std::cout << myCamara.Front.x << "," << myCamara.Front.y << "," << myCamara.Front.x << std::endl;
...
void processInput(GLFWwindow* window)
{
	...
	if (glfwGetKey(window, GLFW_KEY_W) == GLFW_PRESS)
		myCamara.ProcessKeyboard(Camera_Movement::FORWARD, deltaTime);
	if (glfwGetKey(window, GLFW_KEY_S) == GLFW_PRESS)
		myCamara.ProcessKeyboard(Camera_Movement::BACKWARD, deltaTime);
	if (glfwGetKey(window, GLFW_KEY_A) == GLFW_PRESS)
		myCamara.ProcessKeyboard(Camera_Movement::LEFT, deltaTime);
	if (glfwGetKey(window, GLFW_KEY_D) == GLFW_PRESS)
		myCamara.ProcessKeyboard(Camera_Movement::RIGHT, deltaTime);
	if (glfwGetKey(window, GLFW_KEY_Q) == GLFW_PRESS)
		myCamara.ProcessKeyboard(Camera_Movement::UP, deltaTime);
	if (glfwGetKey(window, GLFW_KEY_E) == GLFW_PRESS)
		myCamara.ProcessKeyboard(Camera_Movement::DOWN, deltaTime);
}
...
void mouse_callback(GLFWwindow* window, double xpos, double ypos)
{
	...
	myCamara.ProcessMouseMovement(xoffset, yoffset);
}
...
void scroll_callback(GLFWwindow* window, double xoffset, double yoffset)
{
	myCamara.ProcessMouseScroll(yoffset);
}
